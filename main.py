...
#        print(form.email.data)
        if User.query.filter_by(email=form.email.data).first() != None:
            return render_template("register.html", email_taken=True, form=form)
        else:
            hashed_pw = generate_password_hash(
                form.password.data, salt_length=8)
            new_user = User(
    ...
    if form.validate_on_submit():
        ...
        if not user:
            ...
            if check_password_hash(user.password,form.password.data):
                login_user(user)
                return redirect(request.args["next"] or url_for('get_all_posts'))
            else:
                return render_template("login.html", form=form, incorrect=True)
...
def show_post(post_id):
    ...
    if request.method == "POST":
        new_comm = Comment(
            body = request.form["comment_text"],
